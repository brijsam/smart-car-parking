...
int allSpace = 90;
int countYes = 0;
int carEnter = 16;       /*d0*/               // entry sensor
int carExited = 2;      /*D4*/             //exi sensor
int TRIG = 13;         /*D7*/         //ultrasonic trig  pin
int ECHO = 15;          /*D8*/      // ultrasonic echo pin
int led = 0;         /*D3*/        // spot occupancy signal
int pos;
int pos1;
...
void setup() {
  ...
  Wire.begin(4, 5);      // i2c start D1 pin  D2 pin
  myservo.attach(12);      // servo pin to D6
...
void loop()
{
  ...
  if (distance < 6)
  { //if distance is less than 6cm then on led
    Serial.println("Occupied ");
    digitalWrite(led, HIGH);
  }

  if (distance > 6)
  { //if distance is greater than 6cm then off led
